...
KANBAN_GUIDANCE = (
    ...
    "metadata=..., result=...)`. `summary` is 1–3 human-readable sentences naming "
    "concrete artifacts. `metadata` is machine-readable facts "
    "(`{changed_files: [...], tests_run: N, decisions: [...]}`). "
    "**For content/research/analysis tasks** (answers, reports, findings, "
    "writeups), put the full deliverable in `result` — that's the payload "
    "downstream consumers read. The scratch workspace is GC'd on completion; "
    "anything not in the completion call is lost. Downstream "
    "workers read all three via their own `kanban_show`. Never put secrets / "
    "tokens / raw PII in any field — run rows are durable forever. "

...
KANBAN_GUIDANCE = (
    ...
    "`kanban_create(title=..., body=\"specific scope and acceptance criteria\", "
    "assignee=<right-profile>, parents=[your-task-id])` to spawn a child task "
    "for the appropriate specialist profile instead of scope-creeping into "
    "the next thing. Every `kanban_create` call must include a substantive "
    "`body`; use `triage=True` only when a specifier must write the brief "
    "before dispatch.\n"

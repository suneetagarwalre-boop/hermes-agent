...
- **Task** — a row with title, optional body, one assignee (a profile name), status (`triage | todo | ready | running | blocked | done | archived`), optional tenant namespace, optional idempotency key (dedup for retried automation). A body may be omitted while drafting or triaging, but the dispatcher only spawns `ready` or `review` tasks whose body contains non-whitespace content; blank-body tasks remain queued for an operator to specify.
